...
(async () => {
    ...
    document.getElementById('today-date').textContent = new Date().toLocaleDateString('es-AR', options);
    ...
    function renderResults(students) {
        ...
        const statusConfig = {
            ...
            suspendida: { label: 'Suspendida', classes: 'bg-[#F59E0B]/10 text-[#F59E0B] border-[#F59E0B]/30' },
    ...
    resultsContainer.addEventListener('click', (e) => {
        ...
        renderResults(allStudents.filter(s => s.full_name.toLowerCase().includes(searchInput.value.toLowerCase().trim())));
    ...
    formCheckin.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            }).select('id, check_in_time').single();
    ...
    function renderLogs() {
        ...
        logsContainer.innerHTML = todayLogs.map(log => {
            const time = new Date(log.check_in_time).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' });
            ...
            <div class="log-item">
                <div class="w-8 h-8 rounded-full bg-[#1E293B] flex items-center justify-center text-xs font-bold text-slate-400 overflow-hidden shrink-0">
                    ${avatarSrc ? `<img src="${avatarSrc}" class="w-full h-full object-cover" alt="${escHtml(log.students.full_name || 'Alumno')}">` : escHtml(initials)}
                </div>
                <div class="flex-1">
