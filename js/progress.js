...
(async () => {
  ...
  const MOCK_EXERCISES = [
    { name: 'Sentadilla',     muscle: 'piernas', allTimePR: 100,
      data: [{d:'3 mar',w:80},{d:'7 mar',w:82.5},{d:'11 mar',w:87.5},{d:'14 mar',w:90},
             {d:'18 mar',w:90},{d:'21 mar',w:95},{d:'24 mar',w:92.5},{d:'25 mar',w:95}] },
    { name: 'Press de Banco', muscle: 'pecho',   allTimePR: 75,
      data: [{d:'3 mar',w:60},{d:'7 mar',w:62.5},{d:'11 mar',w:65},{d:'14 mar',w:65},
             {d:'18 mar',w:67.5},{d:'21 mar',w:70},{d:'24 mar',w:70},{d:'25 mar',w:72.5}] },
    { name: 'Peso Muerto',    muscle: 'espalda', allTimePR: 120,
      data: [{d:'3 mar',w:100},{d:'7 mar',w:105},{d:'11 mar',w:110},
             {d:'14 mar',w:115},{d:'18 mar',w:120},{d:'25 mar',w:120}] },
    { name: 'Press Militar',  muscle: 'hombros', allTimePR: 55,
      data: [{d:'3 mar',w:42.5},{d:'7 mar',w:45},{d:'11 mar',w:47.5},
             {d:'18 mar',w:47.5},{d:'21 mar',w:50},{d:'25 mar',w:52.5}] },
    { name: 'Jalón al Pecho', muscle: 'espalda', allTimePR: 65,
      data: [{d:'7 mar',w:55},{d:'11 mar',w:60},{d:'18 mar',w:62.5},{d:'21 mar',w:65}] },
    { name: 'Curl de Bíceps', muscle: 'biceps',  allTimePR: 32.5,
      data: [{d:'3 mar',w:25},{d:'7 mar',w:27.5},{d:'14 mar',w:27.5},{d:'21 mar',w:30},{d:'25 mar',w:32.5}] },
  ];
  ...
  function buildSparkline(data, color) {
    ...
    const areaD = `M ${pts.split(' ')[0].split(',')[0]} ${H} L ${pts.split(' ').map(p => p).join(' L ')} L ${lastX} ${H} Z`;
      ...
      options: {
        ...
        scales: {
          ...
          y: {
            ...
            ticks: { color: '#64748B', font: { size: 10, family: 'IBM Plex Mono' }, callback: v => v + 'kg' }
          }
        }
      }
    });
  ...
  async function loadAnalytics(studentId) {
    ...
    const weekDone = useMock ? MOCK_WEEK_DONE : sessData.filter(s =>
      new Date(s.completed_at) >= weekCutoff
    ).length;
    ...
    document.getElementById('kpi-stag').textContent        = useMock ? 1 : stagData.filter(s => s.is_stagnant).length;
    ...
    renderStagnation(useMock ? [{ exercise_name:'Press de Banco', is_stagnant:true, progress_pct:0, sessions_tracked:3 }] : stagData);
  }
  ...
  function buildExerciseData(logs) {
    ...
    logs.forEach(row => {
      if (!byEx[row.exercise_name]) {
        byEx[row.exercise_name] = { name: row.exercise_name, muscle: row.muscle_group || 'otros', data: [], allTimePR: 0 };
      }
  ...
  function renderConsistency(done, target, sessData, useMock) {
    ...
    document.getElementById('consistency-label').textContent = `${done} sesión${done !== 1 ? 'es' : ''} esta semana`;
  ...
  function renderStagnation(stagData) {
    ...
    progEl.innerHTML = stagnant.slice(0, 2).map(ex => `
  ...
  if (role === 'alumno') {
    let student = null;
    const { data: byProfile } = await db.from('students').select('id, full_name')
      .eq('profile_id', session.user.id).is('deleted_at', null).maybeSingle();
    student = byProfile;

    if (!student && gymId && session.user.email) {
      const { data: byEmail } = await db.from('students').select('id, full_name')
        .eq('gym_id', gymId).eq('email', session.user.email).is('deleted_at', null).maybeSingle();
      student = byEmail;
    }
  ...
  function closePicker() { dropdown?.classList.add('hidden'); }
