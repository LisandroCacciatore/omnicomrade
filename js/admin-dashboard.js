...
function calculateMembershipEndDate(startDate, plan) {
    ...
    const endDateUtc = new Date(Date.UTC(year, (month || 1) - 1, day || 1));
    endDateUtc.setUTCDate(endDateUtc.getUTCDate() + planDays - 1);
    return endDateUtc.toISOString().split('T')[0];
}
...
async function loadKPIs() {
    try {
        ...
        const [
            ...
            db.from('students').select('*', { count: 'exact', head: true }).eq('gym_id', gymId).is('deleted_at', null),
            db.from('students').select('*', { count: 'exact', head: true }).eq('gym_id', gymId).is('deleted_at', null).eq('membership_status', 'activa'),
            db.from('students').select('*', { count: 'exact', head: true }).eq('gym_id', gymId).is('deleted_at', null).eq('membership_status', 'vencida'),
...
window.openModalMembresia = async () => {
    ...
    document.getElementById('membresia-start-date').value = todayISO();
...
async function saveMembresia() {
    ...
    window.tfUtils.setBtnLoading(submitBtn, true, 'Guardando...');
    try {
        const endDate = calculateMembershipEndDate(startDate, plan);
        ...
        if (lastCreatedStudent?.id === studentId) {
            ...
            if (assignProgramModal?.open) assignProgramModal.open({ preStudent: lastCreatedStudent });
            else toast('No se pudo abrir la asignación de programa', 'error');
        }
    } catch (err) {
