...
(async () => {
  ...
  document.querySelectorAll('.q-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll(`.q-btn[data-q="${q}"]`).forEach(b => b.classList.remove(`sel-${q}`))
  ...
  function getScoreLevel(score) {
    if (score >= 70) return { level: 'verde',    label: 'Listo para entrenar',       color: '#10B981', emoji: '🟢' }
    if (score >= 40) return { level: 'amarillo', label: 'Entrenar con precaución',   color: '#F59E0B', emoji: '🟡' }
  ...
  function updateCTA() {
    ...
    if (allDone) {
      ...
      if (numEl)   { numEl.textContent = `${score}/100`; numEl.style.color = color }
      if (barEl)   { barEl.style.width = `${score}%`; barEl.style.background = color }

      btn.style.background = color
      btn.style.color = level === 'rojo' ? '#fff' : '#000'
      btn.textContent = level === 'verde' ? '🔥 ¡A entrenar!'
        : level === 'amarillo' ? '⚠️ Comenzar con precaución'
