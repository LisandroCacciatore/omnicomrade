 ...
 * - showToast -> toast (usar toast directamente)
 * - escapeHtml -> escHtml (usar escHtml directamente)
 * - setButtonLoading -> setBtnLoading (usar setBtnLoading directamente)
 * - openModalLegacy -> showModal (usar showModal)
 * - closeModalLegacy -> hideModal (usar hideModal)
 *
 * Para trackear uso de estos alias, usar window.tfDeprecationTelemetry
 */
...
window.tfUtils = {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape') {
        ...
        if (openPanel) {
          ...
          const backdrop =
            document.getElementById('panel-backdrop') ||
  ...
  showCommandPalette: () => {
    ...
    if (!cp) {
      ...
      searchInput.addEventListener('input', (e) =>
        window.tfUtils.filterCommandPalette(e.target.value)
      );
  ...
  filterCommandPalette: (query) => {
    ...
    if (filtered.length === 0) {
      results.innerHTML =
        '<p class="text-xs text-slate-600 p-4 text-center">No se encontraron resultados</p>';
    }
  },
  ...
  PROGRAMS: [
    {
      ...
      level: 'Principiante',
      description:
        'El programa de fuerza más eficiente para principiantes. Progresión lineal session-a-session con los movimientos fundamentales. Cuando la barra sube cada entrenamiento, no hay sistema más efectivo.',
      stats: [
      ...
      generate(rms) {
        ...
        const sqW_start = pct(rms.sq, 0.55),
          dlW_start = pct(rms.dl, 0.55),
          bpW_start = pct(rms.bp, 0.55),
          ohpW_start = pct(rms.ohp, 0.55),
          pcW_start = pct(rms.pc, 0.65);
        const weeks = [];
        let sqW = sqW_start,
          dlW = dlW_start,
          bpW = bpW_start,
              ...
              ? [
                  {
                    label: 'Sesión A',
                    lifts: [
                      { name: 'Sentadilla', w: sqW, sets: '3×5', incr: 2.5 },
                      { name: 'Press Militar', w: ohpW, sets: '3×5', incr: 1.25 },
                      { name: 'Peso Muerto', w: dlW, sets: '1×5', incr: 2.5 }
                    ]
                  },
                  {
                    label: 'Sesión B',
                    lifts: [
                      { name: 'Sentadilla', w: sqW, sets: '3×5', incr: 2.5 },
                      { name: 'Press Banca', w: bpW, sets: '3×5', incr: 1.25 },
                      { name: 'Power Clean', w: pcW, sets: '5×3', incr: 2.5 }
                    ]
                  },
                  {
                    label: 'Sesión A',
                    lifts: [
                      { name: 'Sentadilla', w: sqW + 2.5, sets: '3×5', incr: 2.5 },
                      { name: 'Press Militar', w: ohpW + 1.25, sets: '3×5', incr: 1.25 },
                      { name: 'Peso Muerto', w: dlW + 2.5, sets: '1×5', incr: 2.5 }
                    ]
                  }
                ]
              : [
                  ...
                  },
                  {
                    label: 'Sesión B',
                    lifts: [
                      { name: 'Sentadilla', w: sqW + 5, sets: '3×5', incr: 2.5 },
                      { name: 'Press Banca', w: bpW + 1.25, sets: '3×5', incr: 1.25 },
                      { name: 'Power Clean', w: pcW + 2.5, sets: '5×3', incr: 2.5 }
                    ]
                  }
                ];
    ...
    },
    {
      ...
      level: 'Principiante',
      description:
        'Dos sesiones alternadas con los 5 movimientos básicos. Volumen mayor que SS con 5×5 en todos los levantamientos principales. Ideal cuando el principiante necesita más práctica técnica acumulando volumen.',
      stats: [
        { icon: 'calendar_month', label: '12 semanas' },
        { icon: 'repeat', label: '3 días/sem' },
        { icon: 'trending_up', label: 'Progresión lineal' },
      ...
      generate(rms) {
        ...
        const { pct } = window.tfUtils;
        const sq_start = pct(rms.sq, 0.5),
          dl_start = pct(rms.dl, 0.5),
          bp_start = pct(rms.bp, 0.5),
          row_start = pct(rms.row, 0.5),
          ohp_start = pct(rms.ohp, 0.5);
        const weeks = [];
        let sq = sq_start,
          dl = dl_start,
          bp = bp_start,
            ...
            days: [
              {
                label: 'Sesión A',
                lifts: [
                  { name: 'Sentadilla', w: sq, sets: '5×5', incr: 2.5 },
                  { name: 'Press Banca', w: bp, sets: '5×5', incr: 1.25 },
                  { name: 'Remo con barra', w: row, sets: '5×5', incr: 1.25 }
                ]
              },
              {
                label: 'Sesión B',
                lifts: [
                  { name: 'Sentadilla', w: sq + 2.5, sets: '5×5', incr: 2.5 },
                  { name: 'Press Militar', w: ohp, sets: '5×5', incr: 1.25 },
                  { name: 'Peso Muerto', w: dl, sets: '1×5', incr: 2.5 }
                ]
              },
              {
                label: 'Sesión A',
                lifts: [
                  { name: 'Sentadilla', w: sq + 5, sets: '5×5', incr: 2.5 },
                  { name: 'Press Banca', w: bp + 1.25, sets: '5×5', incr: 1.25 },
                  { name: 'Remo con barra', w: row + 1.25, sets: '5×5', incr: 1.25 }
                ]
              }
            ]
          });
    ...
    },
    {
      ...
      description:
        'Progresión lineal estructurada en 3 tiers. T1 (fuerza máxima), T2 (hipertrofia de base) y T3 (volumen asistencia). Cuando falla la progresión en T1, cambia la rep scheme antes de reiniciar pesos.',
      stats: [
      ...
      generate(rms) {
        ...
        const weekFn = (w, t1Idx, t2Idx) => {
          const t1s = t1Schemes[Math.min(t1Idx, 2)],
            t2s = t2Schemes[Math.min(t2Idx, 2)];
          const t1Perc = t1Idx === 0 ? 0.85 : t1Idx === 1 ? 0.9 : 0.95;
          const t2Perc = t2Idx === 0 ? 0.65 : t2Idx === 1 ? 0.7 : 0.75;
    ...
    },
    {
      ...
      level: 'Intermedio',
      description:
        'El programa de fuerza a largo plazo más utilizado. Ciclos de 4 semanas (5+, 3+, 1+, deload) sobre el 90% del 1RM real. La clave: respetar el Training Max y acumular más reps que el ciclo anterior.',
      stats: [
      ...
      generate(rms) {
        ...
        return this.weekConfig.map((wk) => {
          ...
          const days = lifts.map((l, i) => {
            ...
            const bbbW = isDeload ? null : pct(l.tm, 0.5);
            return {
              ...
              lifts: [
                ...
                ...(bbbW
                  ...
                  : []),
                {
                  name: 'Asistencia push/pull/core',
                  w: 0,
                  sets: '3-4 series',
                  type: 'ACC',
                  color: '#334155'
    ...
    },
    {
      ...
      description:
        'Rotación de estímulos: nunca todo pesado al mismo tiempo. Cada semana un levantamiento es Pesado, otro Explosivo y otro de Repeticiones. El SNC descansa mientras el cuerpo se adapta.',
      stats: [
      ...
      matrix: {
        SQ: [
          {
            ...
            perc: 0.8,
            color: '#EF4444',
            note: 'Serie tope, máxima tensión'
          },
          {
            type: 'Explosivo',
            sets: '8×3',
            perc: 0.6,
            color: '#3B82F6',
            note: 'Velocidad máxima en la suba'
          },
        ...
        BP: [
          ...
          },
          {
            ...
            perc: 0.6,
            color: '#3B82F6',
            note: 'Pausa de 1 seg en el pecho'
          }
        ],
        DL: [
          {
      ...
      generate(rms) {
        const { round, pct } = window.tfUtils;
        const TM = { sq: round(rms.sq * 0.95), bp: round(rms.bp * 0.95), dl: round(rms.dl * 0.95) };
        const weekFocus = ['Sentadilla Pesada', 'Peso Muerto Pesado', 'Banca Pesada'];

        return [1, 2, 3].map((w) => {
          const sqCfg = this.matrix.SQ[w - 1],
            bpCfg = this.matrix.BP[w - 1],
            dlCfg = this.matrix.DL[w - 1];
          return {
            ...
            days: [
              {
                ...
                lifts: [
                  {
                    name: 'Sentadilla',
                    w: pct(TM.sq, sqCfg.perc),
                    sets: sqCfg.sets,
                    type: sqCfg.type,
                    color: sqCfg.color,
                    sublabel: `${Math.round(sqCfg.perc * 100)}% TM`,
                    note: sqCfg.note
                  },
                  {
              ...
              },
              {
                ...
                lifts: [
                  {
                    name: 'Press Banca',
                    w: pct(TM.bp, bpCfg.perc),
                    sets: bpCfg.sets,
                    type: bpCfg.type,
                    color: bpCfg.color,
                    sublabel: `${Math.round(bpCfg.perc * 100)}% TM`,
                    note: bpCfg.note
                  },
                  {
              ...
              },
              {
                ...
                lifts: [
                  {
                    name: 'Peso Muerto',
                    w: pct(TM.dl, dlCfg.perc),
                    sets: dlCfg.sets,
                    type: dlCfg.type,
                    color: dlCfg.color,
                    sublabel: `${Math.round(dlCfg.perc * 100)}% TM`,
                    note: dlCfg.note
                  },
                  {
    ...
    },
    {
      ...
      level: 'Intermedio',
      description:
        'División Push/Pull/Legs repetida dos veces por semana. Máximo volumen para hipertrofia con alta frecuencia por grupo muscular. Sin necesidad de 1RM específico — se trabaja por RPE y peso propio.',
      stats: [
            ...
            lifts: this.sessions[type].map((ex) => {
              const base =
                type === 'PUSH'
                  ...
                  : type === 'PULL'
                    ? rms.row
                    : type === 'LEGS' && ex.name.includes('Sentadilla')
                      ? rms.sq
                      : type === 'LEGS' && ex.name.includes('Rumano')
                        ? rms.rdl
                        : type === 'LEGS'
                          ? rms.sq * 0.6
                          : rms.bp;
  ...
  setBtnLoading: (btnId, isLoading, loadingText = 'Procesando...') =>
    window.tfUiUtils?.setBtnLoading
  ...
  setLoading(btn, isLoading, loadingText = 'Guardando...') {
    if (isLoading) {
      btn.disabled = true;
      btn._originalText =
        btn.querySelector('span:not(.animate-spin)')?.textContent || btn.textContent;
  ...
  initModalAccessibility: (modalId, closeBtnId, backdropId) => {
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.attributeName === 'class') {
          const isNowVisible =
            !modal.classList.contains('hidden') && modal.classList.contains('modal-active');
          if (isNowVisible) {
    ...
    const backdrop = backdropId
      ...
      : modal.querySelector('[id*="-backdrop"]');
    if (backdrop) {
  ...
  _initShortcuts: () => {
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
        ...
        if (m) window.tfUtils.hideModal(m.id);
      }
      if (e.key === '/' && e.target.tagName !== 'INPUT' && e.target.tagName !== 'TEXTAREA') {
        const s = document.querySelector('input[id*="search"]');
        if (s) {
          e.preventDefault();
          s.focus();
        }
  ...
  _filterCP: (q) => {
    const r = document.getElementById('cp-results');
    if (!r) return;
    const items = [
      { l: 'Ir a Alumnos', i: 'group', h: 'student-list.html' },
      { l: 'Ir a Membresías', i: 'card_membership', h: 'membership-list.html' },
      { l: 'Ir a Rutinas', i: 'fitness_center', h: 'routine-list.html' },
      { l: 'Ir a Ejercicios', i: 'sports_gymnastics', h: 'exercise-list.html' },
      { l: 'Ir a Admin', i: 'admin_panel_settings', h: 'admin-dashboard.html' },
      { l: 'Nueva Membresía', i: 'add_card', a: 'openModal("modal-nueva-membresia")' },
      { l: 'Nuevo Alumno', i: 'person_add', a: 'openModal("modal-nuevo-alumno")' },
      { l: 'Cerrar Sesión', i: 'logout', a: 'window.tfUtils.logout()' }
    ];
    const f = items.filter((i) => i.l.toLowerCase().includes(q.toLowerCase()));
    r.innerHTML = f
      .map(
        (i) =>
          `<button class="cp-item w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 text-left" ${i.h ? `onclick="location.href='${i.h}'"` : `onclick="${i.a};window.tfUtils.hideCommandPalette()"`}><span class="material-symbols-rounded text-slate-500">${i.i}</span><span class="text-slate-200">${i.l}</span></button>`
      )
      .join('');
  },
  _showModal: (id) => {
  ...
  _setLoading: (btnId, isLoading, loadingText) => {
    const btn = typeof btnId === 'string' ? document.getElementById(btnId) : btnId;
    if (!btn) return;
    const textEl = btn.querySelector('[id$="-text"]') || btn;
    const spinner = btn.querySelector('[id$="-spinner"]');
  ...
  _initAcc: (modalId, closeBtnId, backdropId) => {
    const m = document.getElementById(modalId);
    const c = document.getElementById(closeBtnId);
    if (!m || !c) return;
    c.addEventListener('click', () => window.tfUtils.hideModal(modalId));
    m.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') window.tfUtils.hideModal(modalId);
    });
    const o = new MutationObserver((ms) =>
      ms.forEach((mu) => {
        if (
          ...
          m.classList.contains('modal-active')
        ) {
          const f = m.querySelector(
            'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
          );
          if (f) setTimeout(() => f.focus(), 150);
        }
      })
    );
    o.observe(m, { attributes: true });
    const b = backdropId
      ? document.getElementById(backdropId)
      : m.querySelector('[id*="-backdrop"]');
    if (b) b.addEventListener('click', () => window.tfUtils.hideModal(modalId));
  },
  _setupVal: (inputEl, errorEl, validator) => {
    if (!inputEl || !errorEl) return;
    const validate = () => {
  ...
  _focusTrap: (modal) => {
    ...
    const handleEsc = (e) => {
      if (e.key === 'Escape') modal.dispatchEvent(new CustomEvent('tf:close'));
    };
  ...
  _setLoad: (btn, isLoading, loadingText) => {
    if (isLoading) {
      btn.disabled = true;
      btn._originalText =
        btn.querySelector('span:not(.animate-spin)')?.textContent || btn.textContent;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  knownModals.forEach((m) => {
    if (document.getElementById(m.id)) window.tfUtils.initModalAccessibility(m.id, m.close);
  });
});
