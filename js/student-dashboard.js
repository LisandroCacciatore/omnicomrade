...
(async () => {
  ...
  const { escHtml, logout } = window.tfUtils;
  ...
  if (isStaff && targetId) {
    ...
    backBtn.className = 'flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-white transition-colors px-4 pt-4 -mb-2';
    backBtn.innerHTML = '<span class="material-symbols-rounded text-[16px]">arrow_back</span> Volver a alumnos';
    document.querySelector('main')?.prepend(backBtn);
  }

  /* ─── Resolver student ─────────────────────────────────── */
  let student = null;
  const STUDENT_SELECT = 'id, full_name, email, routine_id, membership_status, medical_certificate_url, coach_notes, gyms(name)';

  if (isStaff && targetId) {
    const { data } = await db.from('students').select(STUDENT_SELECT)
      .eq('id', targetId).is('deleted_at', null).maybeSingle();
    student = data;
  } else {
    const { data: byProfile } = await db.from('students').select(STUDENT_SELECT)
      .eq('profile_id', userId).is('deleted_at', null).maybeSingle();
    student = byProfile;

    if (!student && gymId && session.user.email) {
      const { data: byEmail } = await db.from('students').select(STUDENT_SELECT)
        .eq('gym_id', gymId).eq('email', session.user.email)
        .is('deleted_at', null).maybeSingle();
      if (byEmail) {
        student = byEmail;
        try { db.from('students').update({ profile_id: userId }).eq('id', byEmail.id).is('profile_id', null).then(() => {}); } catch(_) {}
      }
    }
  }
  ...
  const [
    ...
    db.from('memberships').select('plan, end_date, start_date')
      .eq('student_id', student.id)
      .order('end_date', { ascending: false }).limit(1).maybeSingle(),

    db.from('student_programs').select(`
      id, current_week, started_at, status,
      program_templates(id, slug, name, weeks, days_per_week, level)
    `).eq('student_id', student.id).eq('status', 'activo').maybeSingle(),

    student.routine_id
      ? db.from('routines').select('id, name, objetivo, days_per_week').eq('id', student.routine_id).maybeSingle()
      : Promise.resolve({ data: null }),

    db.from('profiles').select('full_name').eq('gym_id', gymId)
      .eq('role', 'profesor').limit(1).maybeSingle(),

    db.from('workout_sessions').select('id, completed_at, day_name, duration_minutes')
      .eq('student_id', student.id).not('completed_at', 'is', null)
      .gte('completed_at', monthStart.toISOString())
      .order('completed_at', { ascending: false }),

    db.from('wellbeing_logs').select('sleep, pain, energy, checked_at')
      .eq('student_id', student.id)
  ...
  const daysPerWeek = activeProg?.program_templates?.days_per_week
    || routine?.data?.days_per_week || routine?.days_per_week || 4;
  const sessThisWeek = sessData.filter(s => new Date(s.completed_at) >= weekStart).length;

  /* ─── CTA Entrenar ──────────────────────────────────── */
  const routineName = activeProg?.program_templates?.name
    || routine?.data?.name || routine?.name || 'Mi Rutina';
  ...
  document.getElementById('cta-day-info').textContent = daysPerWeek > 0
  ...
  if (activeProg) {
    ...
    document.getElementById('plan-meta').textContent = `Semana ${week} de ${total} · ${tpl.level || ''}`;
  } else if (routine?.name || routine?.data?.name) {
    const r = routine?.data || routine;
    document.getElementById('plan-name').textContent = r.name;
    document.getElementById('plan-meta').textContent = `${r.objetivo || 'general'} · ${r.days_per_week || '—'} días/sem`;
  } else {
    document.getElementById('plan-meta').textContent = 'Sin plan asignado — contactá a tu entrenador';
  }
  ...
  const sessionDates = new Set(
    sessData.filter(s => new Date(s.completed_at) >= weekStart)
            .map(s => new Date(s.completed_at).toDateString())
  );

  const dotsHTML = dayNames.map((d, i) => {
    const dayDate = new Date(weekStart);
    dayDate.setDate(dayDate.getDate() + i);
    const trained  = sessionDates.has(dayDate.toDateString());
    const isToday  = i === todayDow;
    const cls = `day-dot${trained ? ' trained' : ''}${isToday ? ' is-today' : ''}`;
    const colCls = isToday && trained ? 'day-dot-col trained-today'
                 : isToday           ? 'day-dot-col is-today'
                 : 'day-dot-col';
  ...
  if (!current.length) {
    ...
    const avg = (arr, key) => arr.length ? arr.reduce((a, w) => a + (w[key] || 0), 0) / arr.length : null;
  ...
  if (window.AthleteInsights) {
    const scoreData  = window.AthleteInsights.calcAthleteScore({
      wbLogs: wbData, sessions: sessData, logs: [], daysPerWeek,
    });
    const prediction = window.AthleteInsights.predictSessionQuality({
      wbLogs: wbData, logs: [], sessions: sessData,
    });
  ...
  function renderAthleteScoreWidget(scoreData, prediction) {
    ...
    const predBanner = prediction.risk > 35 ? `
