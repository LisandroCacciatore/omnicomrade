...
(async () => {
    ...
    async function loadRoutines() {
        const [{ data, error }, { data: assignments }] = await Promise.all([
            ...
            db.from('students').select('routine_id')
              .eq('gym_id', gymId).is('deleted_at', null)
              .not('routine_id', 'is', null),
        ]);

        if (error) { toast('Error al cargar rutinas', 'error'); return; }
    ...
    function updateKPIs() {
        ...
        document.getElementById('kpi-fuerza').textContent = allRoutines.filter(r => r.objetivo === 'fuerza').length;
        document.getElementById('kpi-estetica').textContent = allRoutines.filter(r => r.objetivo === 'estetica').length;
        document.getElementById('kpi-rendimiento').textContent = allRoutines.filter(r => r.objetivo === 'rendimiento').length;
    }
    ...
    function routineCard(r) {
        ...
        const objLabel = { fuerza: 'Fuerza', estetica: 'Estética', rendimiento: 'Rendimiento', rehabilitacion: 'Rehab.', general: 'General' }[obj] || obj;
        const diffLabel = { principiante: 'Principiante', intermedio: 'Intermedio', avanzado: 'Avanzado' }[diff] || diff;

        // Color coding badges
        const objStyles = {
            fuerza: 'bg-red-500/15 text-red-400', estetica: 'bg-pink-500/15 text-pink-400',
            rendimiento: 'bg-blue-500/15 text-blue-400', rehabilitacion: 'bg-emerald-500/15 text-emerald-400',
            general: 'bg-slate-500/15 text-slate-400'
        };
        const diffStyles = { principiante: 'bg-emerald-500/15 text-emerald-400', intermedio: 'bg-amber-500/15 text-amber-400', avanzado: 'bg-red-500/15 text-red-400' };
    ...
    document.querySelectorAll('.objetivo-btn').forEach(btn => {
        btn.addEventListener('click', () => { selectedObjetivo = btn.dataset.val; syncObjetivoUI(); });
    });
    document.querySelectorAll('.diff-btn').forEach(btn => {
        btn.addEventListener('click', () => { selectedDiff = btn.dataset.val; syncDiffUI(); });
    });

    function syncObjetivoUI() {
        document.querySelectorAll('.objetivo-btn').forEach(b => b.classList.toggle('active', b.dataset.val === selectedObjetivo));
    }
    function syncDiffUI() {
        document.querySelectorAll('.diff-btn').forEach(b => {
            b.className = 'diff-btn flex-1 py-2 rounded-xl text-xs font-semibold border border-[#1E293B] bg-[#0B1218] text-slate-400 transition-colors';
