...
class ProgramAssignModal {
  ...
  /* ── Helpers ─────────────────────────────────────────────── */
  _backdropEl() {
    return document.getElementById('pa-backdrop');
  }
  _modalEl() {
    return document.getElementById('pa-modal');
  }
  _bodyEl() {
    return document.getElementById('pa-body');
  }
  _footerEl() {
    return document.getElementById('pa-footer');
  }
  ...
  _render() {
    ...
    document.getElementById('pa-step-bar').innerHTML = stepLabels
      .map((l, i) => {
        const stepNum = i + 1;
  ...
  _renderStudentList(query) {
    ...
    const statusColor = {
      activa: 'text-[#10B981] border-[#10B981]/30 bg-[#10B981]/10',
      vencida: 'text-[#EF4444] border-[#EF4444]/30 bg-[#EF4444]/10',
      suspendida: 'text-[#F59E0B] border-[#F59E0B]/30 bg-[#F59E0B]/10',
      pendiente: 'text-slate-400 border-slate-500/30 bg-slate-800'
    };
  ...
  async _save() {
    ...
    try {
      ...
      const rmValues = {};
      this._selProgram.inputs.forEach((i) => {
        rmValues[i.id] = this._rms[i.id] || i.default;
      });
      ...
      if (nextBtn) {
        ...
        nextBtn.textContent = 'Confirmar asignación ✓';
      }
      ...
      footer.innerHTML = `
        <div class="flex-1 text-left">
          ...
          <a href="program-assign.html?student=${this._selStudent?.id}" 
        ...
        <button type="button" onclick="this.closest('form').querySelector('#pa-next').click()" 
