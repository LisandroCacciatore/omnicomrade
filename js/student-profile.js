...
(async () => {
  ...
  async function loadRoutineById(routineId) {
    ...
    const { data } = await db.from('routines').select(`
  ...
  async function fetchStudent(filter) {
    let { data, error } = await db.from('students').select(STUDENT_SELECT)
      .match(filter).is('deleted_at', null).maybeSingle();
    if (error && error.code === '42703') {
      // routine_id no existe aún → fallback sin esa columna
      ({ data, error } = await db.from('students').select(STUDENT_SELECT_FALLBACK)
        .match(filter).is('deleted_at', null).maybeSingle());
    }
    return data;
  }
    ...
    if (!student) {
      ...
      if (gymId && userEmail) {
        ...
        if (byEmail) {
          ...
          db.from('students').update({ profile_id: session.user.id }).eq('id', byEmail.id).is('profile_id', null).then(() => {});
  ...
  if (!student) {
    ...
    if (area) area.innerHTML = buildNoProgram('No se encontró el alumno', 'El ID proporcionado no corresponde a un alumno activo.');
    return;
  }

  // 3. Cargar perfil para nombre y gimnasio (no-fatal si no existe)
  if (student.profile_id) {
    const { data: prof } = await db.from('profiles').select('id, full_name, gym_id, gyms(name)').eq('id', student.profile_id).maybeSingle();
    profile = prof;
  }
  ...
  let painBadge = '';
  if (student) {
      const todayDateStr = new Date().toLocaleDateString('en-CA', { timeZone: 'America/Argentina/Buenos_Aires' });
      const { data: todayLog } = await db.from('wellbeing_logs').select('pain, pain_zone').eq('student_id', student.id).eq('check_date', todayDateStr).maybeSingle();
  ...
  if (studentNameHeader) studentNameHeader.innerHTML = `${escHtml(profile?.full_name || student.full_name || 'Alumno')}${painBadge}`;
  if (gymNameHeader) gymNameHeader.textContent = profile?.gyms?.name || student.gyms?.name || 'TechFitness';

  const MUSCLE_COLORS = { pecho:'#F97316', espalda:'#3B82F6', hombros:'#8B5CF6', biceps:'#EC4899', triceps:'#F59E0B', core:'#EF4444', piernas:'#10B981', gluteos:'#06B6D4', cardio:'#84CC16', otros:'#64748B' };
  ...
  const { data: ap } = await db.from('student_programs').select(`id, status, current_week, started_at, rm_values, program_templates (id, slug, name, description, weeks, days_per_week, level)`).eq('student_id', student.id).eq('status', 'activo').maybeSingle();
  activeProgram = ap;
  ...
  if (area) {
    if (activeProgram) {
      ...
      area.innerHTML = buildNoProgram('Sin rutina asignada', 'Todavía no hay programas o rutinas activas para este entrenamiento.');
    }
  }
  ...
  async function loadStatsData(studentId) {
    ...
    const { data: sessions } = await db
      .from('workout_sessions')
      .select('id, completed_at, day_name, workout_exercise_logs(exercise_name, weight_used, set_number, status)')
      .eq('student_id', studentId)
      .not('completed_at', 'is', null)
    ...
    if (!sessions || sessions.length === 0) {
      statsArea.innerHTML = painMapHTML + buildStatsEmpty('Sin sesiones completadas', 'Completá tu primer entrenamiento para ver el seguimiento aquí.');
      return;
    }
    ...
    if (!exercises.length) {
      statsArea.innerHTML = buildStatsEmpty('Sin datos de peso', 'Registrá pesos en tu entreno para ver la evolución.');
      return;
    }

    const dateStr = new Date(lastSession.completed_at).toLocaleDateString('es-AR', { weekday:'long', day:'numeric', month:'long' });
    const prevDateStr = prevSession ? new Date(prevSession.completed_at).toLocaleDateString('es-AR', { day:'numeric', month:'short' }) : null;

    const rows = exercises.sort().map(ex => {
      const current = lastLogs[ex];
      const prev    = prevLogs[ex];
      let delta = null, color = '#64748B', icon = '→', bgColor = 'rgba(100,116,139,.08)', borderColor = '#1E293B';

      if (prev && prev > 0) {
        delta = current - prev;
        if (delta > 0) {
          color = '#10B981'; icon = '↑'; bgColor = 'rgba(16,185,129,.06)'; borderColor = 'rgba(16,185,129,.25)';
        } else if (delta < 0) {
          color = '#EF4444'; icon = '↓'; bgColor = 'rgba(239,68,68,.06)'; borderColor = 'rgba(239,68,68,.25)';
        } else {
          color = '#F59E0B'; icon = '→'; bgColor = 'rgba(245,158,11,.06)'; borderColor = 'rgba(245,158,11,.2)';
        }
      } else if (!prev && current > 0) {
        // Primera vez — marcar como nuevo
        color = '#3B82F6'; icon = '★'; bgColor = 'rgba(59,130,246,.06)'; borderColor = 'rgba(59,130,246,.2)';
      }

      const deltaLabel = delta !== null
        ? `<span style="font-size:10px;font-weight:800;color:${color};margin-left:4px">${delta > 0 ? '+' : ''}${delta}kg</span>`
        : prev === undefined ? `<span style="font-size:9px;color:#475569;margin-left:4px">1ª vez</span>` : '';
    ...
    statsArea.innerHTML = painMapHTML + `
  ...
  function buildProgramView(prog, completedToday = new Set()) {
    ...
    const icon = { 'starting-strength':'🚂','stronglifts-5x5':'🏗️','wendler-531':'📈','cube-method':'🧊','gzclp':'⚡','ppl':'🔄' }[tpl.slug] || '🏋️';
  ...
  function buildRoutineView(routine, completedToday = new Set()) {
    const days = (routine.routine_days || []).sort((a,b) => a.day_number - b.day_number).map(rd => ({
        name: rd.name || `Día ${rd.day_number}`,
        exercises: (rd.routine_day_exercises || []).sort((a,b) => a.order_index - b.order_index).map(rde => ({
            name: rde.exercises?.name || rde.exercise_name || '—',
            exercise_id: rde.exercises?.id || null,
            muscle: rde.exercises?.muscle_group,
            sets: rde.sets, reps: rde.reps, rest: rde.rest_seconds,
            rpe: rde.rpe, weight_kg: rde.weight_kg, weight_ref: rde.weight_ref, note: rde.notes,
            sets_data: (rde.routine_day_exercise_sets || []).sort((a,b) => a.set_number - b.set_number),
        }))
    }));
    const oc = { fuerza:'#EF4444', estetica:'#EC4899', rendimiento:'#3B82F6', rehabilitacion:'#10B981', general:'#64748B' }[routine.objetivo] || '#64748B';
  ...
  function buildDayPanel(day, idx, rms, routineName, completedToday = new Set()) {
    ...
    day.exercises.forEach(ex => {
      ...
      const sd = ex.sets_data && ex.sets_data.length > 0
        ? ex.sets_data
  ...
  function buildExCard(ex, rms) {
    ...
    if (hasSets) {
      const setsHTML = ex.sets_data.map(s => {
        const w = s.weight_kg != null ? `${s.weight_kg}<span class="text-[9px] text-slate-500 ml-0.5">kg</span>` : `<span class="text-slate-600">—</span>`;
        const rpeStr = s.rpe_target != null ? `<span class="text-[9px] font-mono text-amber-400">RPE ${s.rpe_target}</span>` : '';
        const rirStr = s.rir_target != null ? `<span class="text-[9px] font-mono text-slate-500">RIR ${s.rir_target}</span>` : '';
        const amrap  = s.is_amrap ? `<span class="text-[9px] font-bold text-amber-400 bg-amber-500/10 px-1 rounded">AMRAP</span>` : '';
    ...
    const weightDisplay = ex.weight_kg > 0
      ? `<span class="font-mono text-[#60A5FA] text-sm">${ex.weight_kg}</span><span class="text-[9px] text-slate-500 ml-0.5">KG</span>`
      : `<span class="text-slate-600 text-xs">—</span>`;
    const rpeHTML = ex.rpe ? `<span class="text-[9px] font-bold font-mono px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20">RPE ${ex.rpe}</span>` : '';
    return `
    <div class="bg-[#161E26] border border-[#1E293B] rounded-2xl overflow-hidden hover:border-[#334155] transition-colors">
      <div class="p-3 flex items-center gap-2.5 border-b border-[#0B1218]">
        <div class="w-1 h-9 rounded-full shrink-0" style="background:${mc}"></div>
  ...
  function bindDayTabs() {
    ...
    tabsContainer.addEventListener('click', e => {
        ...
        panelsContainer.querySelector(`.day-panel[data-day-idx="${idx}"]`)?.classList.remove('hidden');
    });
  }
  ...
  function getProgramDays(slug, rms, week) {
    ...
    switch (slug) {
      case 'starting-strength': {
        const sq = applyFactor(pct(rms.sq||60,.55)), dl = applyFactor(pct(rms.dl||80,.55)), bp = applyFactor(pct(rms.bp||50,.55)), ohp = applyFactor(pct(rms.ohp||35,.55)), pc = applyFactor(pct(rms.pc||30,.65));
        return [{ name:'Sesión A', exercises:[{ name:'Sentadilla', muscle:'piernas', sets:3, reps:'5', rest:180, weight_kg:sq },{ name:'Press Militar', muscle:'hombros', sets:3, reps:'5', rest:120, weight_kg:ohp },{ name:'Peso Muerto', muscle:'espalda', sets:1, reps:'5', rest:180, weight_kg:dl }] }, { name:'Sesión B', exercises:[{ name:'Sentadilla', muscle:'piernas', sets:3, reps:'5', rest:180, weight_kg:sq },{ name:'Press Banca', muscle:'pecho', sets:3, reps:'5', rest:120, weight_kg:bp },{ name:'Power Clean', muscle:'espalda', sets:5, reps:'3', rest:120, weight_kg:pc }] }];
      }
      case 'stronglifts-5x5': {
        const sq = applyFactor(pct(rms.sq||60,.50)), ohp = applyFactor(pct(rms.ohp||30,.50)), bp = applyFactor(pct(rms.bp||45,.50)), row = applyFactor(pct(rms.row||45,.50)), dl = applyFactor(pct(rms.dl||80,.50));
        return [{ name:'Sesión A', exercises:[{ name:'Sentadilla', muscle:'piernas', sets:5, reps:'5', rest:180, weight_kg:sq },{ name:'Press Banca', muscle:'pecho', sets:5, reps:'5', rest:120, weight_kg:bp },{ name:'Remo con Barra', muscle:'espalda', sets:5, reps:'5', rest:120, weight_kg:row }] }, { name:'Sesión B', exercises:[{ name:'Sentadilla', muscle:'piernas', sets:5, reps:'5', rest:180, weight_kg:sq },{ name:'Press Militar', muscle:'hombros', sets:5, reps:'5', rest:120, weight_kg:ohp },{ name:'Peso Muerto', muscle:'espalda', sets:1, reps:'5', rest:180, weight_kg:dl }] }];
      }
      case 'wendler-531': {
        const wc = [{ percs:[.65,.75,.85], reps:['5','5','5+'], name:'Vol 5+' },{ percs:[.70,.80,.90], reps:['3','3','3+'], name:'Hib 3+' },{ percs:[.75,.85,.95], reps:['5','3','1+'], name:'Pico 1+' },{ percs:[.40,.50,.60], reps:['5','5','5'], name:'Deload' }][(w-1) % 4];
        const TM = { sq:round((rms.sq||120)*.9), bp:round((rms.bp||90)*.9), dl:round((rms.dl||160)*.9), ohp:round((rms.ohp||60)*.9) };
        const top = (tm, pi) => round(tm * wc.percs[pi]);
        return [{ name:`Día 1 — SQ`, exercises:[{ name:'Sentadilla', muscle:'piernas', sets:1, reps:wc.reps[0], rest:180, weight_kg:top(TM.sq,0) },{ name:'Sentadilla', muscle:'piernas', sets:1, reps:wc.reps[1], rest:180, weight_kg:top(TM.sq,1) },{ name:'Sentadilla (top)', muscle:'piernas', sets:1, reps:wc.reps[2], rest:180, weight_kg:top(TM.sq,2), rpe:9 },{ name:'Sentadilla (BBB)', muscle:'piernas', sets:5, reps:'10', rest:90, weight_kg:round(TM.sq*.5) }] }, { name:`Día 2 — BP`, exercises:[{ name:'Press Banca', muscle:'pecho', sets:1, reps:wc.reps[0], rest:180, weight_kg:top(TM.bp,0) },{ name:'Press Banca', muscle:'pecho', sets:1, reps:wc.reps[1], rest:180, weight_kg:top(TM.bp,1) },{ name:'Press Banca (top)', muscle:'pecho', sets:1, reps:wc.reps[2], rest:180, weight_kg:top(TM.bp,2), rpe:9 },{ name:'Press Banca (BBB)', muscle:'pecho', sets:5, reps:'10', rest:90, weight_kg:round(TM.bp*.5) }] }];
      }
      // ── GZCLP ──────────────────────────────────────────────────
      case 'gzclp': {
        ...
        const t1s = t1Schemes[t1Idx], t2s = t2Schemes[t2Idx];
        const t1Perc = [.85, .90, .95][t1Idx];
        const t2Perc = [.65, .70, .75][t2Idx];
        ...
        return [
          { name:'Día A — Lower+Upper', exercises:[
            { name:'Sentadilla [T1]',        muscle:'piernas',  sets:5, reps:t1s, rest:240, weight_kg:sq,  rpe:9 },
            { name:'Press Horizontal [T2]',  muscle:'pecho',    sets:3, reps:t2s, rest:120, weight_kg:bpT2 },
            { name:'Asistencia [T3]',        muscle:'otros',    sets:3, reps:'15+', rest:60, weight_kg:0   },
          ]},
          { name:'Día B — Upper+Lower', exercises:[
            { name:'Press Horiz. [T1]',      muscle:'pecho',    sets:5, reps:t1s, rest:240, weight_kg:bp,  rpe:9 },
            { name:'Hip Hinge [T2]',         muscle:'espalda',  sets:3, reps:t2s, rest:120, weight_kg:hpT2 },
            { name:'Asistencia [T3]',        muscle:'otros',    sets:3, reps:'15+', rest:60, weight_kg:0   },
          ]},
          { name:'Día C — Lower+Upper', exercises:[
            { name:'Hip Hinge [T1]',         muscle:'espalda',  sets:5, reps:t1s, rest:240, weight_kg:hp,  rpe:9 },
            { name:'Sentadilla [T2]',        muscle:'piernas',  sets:3, reps:t2s, rest:120, weight_kg:sqT2 },
            { name:'Asistencia [T3]',        muscle:'otros',    sets:3, reps:'15+', rest:60, weight_kg:0   },
          ]},
          { name:'Día D — Upper+Lower', exercises:[
            { name:'Pull Vertical [T1]',     muscle:'espalda',  sets:5, reps:t1s, rest:240, weight_kg:pul, rpe:9 },
            { name:'Press Horizontal [T2]',  muscle:'pecho',    sets:3, reps:t2s, rest:120, weight_kg:bpT2 },
            { name:'Asistencia [T3]',        muscle:'otros',    sets:3, reps:'15+', rest:60, weight_kg:0   },
          ]},
        ];
      ...
      // ── Cube Method ────────────────────────────────────────────
      case 'cube-method': {
        const TM = { sq: round((rms.sq||140)*.95), bp: round((rms.bp||105)*.95), dl: round((rms.dl||185)*.95) };
        ...
        const cubeSet = (name, muscle, tm, type) => {
          if (type === 'Pesado')    return { name, muscle, sets:5, reps:'2',  rest:300, weight_kg:applyFactor(pct(tm,.80)), rpe:9 };
          if (type === 'Explosivo') return { name, muscle, sets:8, reps:'3',  rest:120, weight_kg:applyFactor(pct(tm,.60)), rpe:7 };
          /* Reps */                return { name, muscle, sets:1, reps:'8+', rest:180, weight_kg:applyFactor(pct(tm,.70)), rpe:8 };
        };

        return [
          { name:'Día 1 — Sentadilla', exercises:[
            cubeSet('Sentadilla', 'piernas', TM.sq, rotation.sq),
            { name:'Asistencia tren inferior', muscle:'piernas', sets:3, reps:'10-12', rest:90, weight_kg:0 },
          ]},
          { name:'Día 2 — Press Banca', exercises:[
            cubeSet('Press Banca', 'pecho', TM.bp, rotation.bp),
            { name:'Asistencia tren superior', muscle:'pecho', sets:3, reps:'10-12', rest:90, weight_kg:0 },
          ]},
          { name:'Día 3 — Peso Muerto', exercises:[
            cubeSet('Peso Muerto', 'espalda', TM.dl, rotation.dl),
            { name:'Asistencia posterior', muscle:'espalda', sets:3, reps:'10-12', rest:90, weight_kg:0 },
          ]},
        ];
      }
      ...
      case 'ppl': {
        ...
        const bp  = applyFactor(pct((rms.bp  || 80)  * progFactor, .72));
        const inc = applyFactor(pct((rms.bp  || 80)  * progFactor, .65));
        const ohp = applyFactor(pct((rms.bp  || 80)  * progFactor, .62));
        const row = applyFactor(pct((rms.row || 80)  * progFactor, .72));
        const sq  = applyFactor(pct((rms.sq  || 100) * progFactor, .75));
        const rdl = applyFactor(pct((rms.rdl || 90)  * progFactor, .72));

        return [
          { name:'Push 1', exercises:[
            { name:'Press Banca',       muscle:'pecho',    sets:4, reps:'8-12', rest:180, weight_kg:bp  },
            { name:'Press Inclinado',   muscle:'pecho',    sets:3, reps:'10-12',rest:120, weight_kg:inc },
            { name:'Press Militar',     muscle:'hombros',  sets:4, reps:'8-12', rest:120, weight_kg:ohp },
            { name:'Elevaciones Lat.',  muscle:'hombros',  sets:4, reps:'12-15',rest:60,  weight_kg:0   },
            { name:'Extensión Tríceps', muscle:'triceps',  sets:3, reps:'12-15',rest:60,  weight_kg:0   },
          ]},
          { name:'Pull 1', exercises:[
            { name:'Jalón al pecho',   muscle:'espalda',  sets:4, reps:'8-12', rest:180, weight_kg:0   },
            { name:'Remo con barra',   muscle:'espalda',  sets:4, reps:'8-10', rest:180, weight_kg:row },
            { name:'Remo en cable',    muscle:'espalda',  sets:3, reps:'12',   rest:90,  weight_kg:0   },
            { name:'Curl con barra',   muscle:'biceps',   sets:4, reps:'10-12',rest:60,  weight_kg:0   },
            { name:'Curl martillo',    muscle:'biceps',   sets:3, reps:'12',   rest:60,  weight_kg:0   },
          ]},
          { name:'Legs 1', exercises:[
            { name:'Sentadilla',           muscle:'piernas', sets:4, reps:'6-10', rest:240, weight_kg:sq  },
            { name:'Prensa',               muscle:'piernas', sets:3, reps:'10-12',rest:120, weight_kg:0   },
            { name:'P. Muerto Rumano',     muscle:'gluteos', sets:4, reps:'8-10', rest:180, weight_kg:rdl },
            { name:'Extensión cuádriceps', muscle:'piernas', sets:3, reps:'12-15',rest:60,  weight_kg:0   },
            { name:'Curl femoral',         muscle:'piernas', sets:3, reps:'12-15',rest:60,  weight_kg:0   },
          ]},
          ...
          { name:'Legs 2', exercises:[
            ...
            { name:'P. Muerto Rumano',     muscle:'gluteos', sets:4, reps:'8-10', rest:180, weight_kg:rdl },
            { name:'Extensión cuádriceps', muscle:'piernas', sets:3, reps:'12-15',rest:60,  weight_kg:0   },
            { name:'Elevación talones',    muscle:'piernas', sets:4, reps:'15-20',rest:45,  weight_kg:0   },
          ]},
        ];
      }

      default: return [{ name:'Día 1', exercises:[] }];
    }
  }
  ...
  area?.addEventListener('click', e => {
      ...
      if (!startBtn) return;
      const payload = { routineName: startBtn.dataset.routineName, dayName: startBtn.dataset.dayName, exercises: JSON.parse(startBtn.dataset.exercises.replace(/&apos;/g, "'")) };
