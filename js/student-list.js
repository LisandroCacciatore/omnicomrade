/**
 * student-list.js
 * TechFitness — Gestión de Alumnos
 */
...
async function loadStudents(filters = {}) {
    ...
    if (error) { window.tfUtils.toast('Error cargando alumnos', 'error'); console.error(error); return; }
...
function renderTable(students) {
    ...
    tbody.innerHTML = students.map(s => {
        const latestMembership = s.memberships
        ...
        if (latestMembership) {
            ...
            const dateStr = end.toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit', year: 'numeric' });
            const colorClass = diffDays < 0 ? 'text-danger' : diffDays <= 7 ? 'text-warning' : 'text-slate-400';
...
function setupTableEvents() {
    ...
    tbody.addEventListener('click', (e) => {
        ...
        if (actionBtn) {
            ...
            if (action === 'edit') openEditAlumno(id);
...
async function renderTabRutina(studentId, studentName) {
    ...
    const [
        ...
        db.from('student_programs')
            .select(`id, status, current_week, started_at, rm_values,
                   program_templates (id, slug, name, description, weeks, days_per_week, level)`)
            .eq('student_id', studentId).eq('status', 'activo').maybeSingle(),
        db.from('student_programs')
            .select('id, status, started_at, program_templates(name)')
            .eq('student_id', studentId).neq('status', 'activo')
            .order('created_at', { ascending: false }).limit(3),
        db.from('students').select('id, routine_id, routines(id, name, objetivo, days_per_week, duration_weeks)')
            .eq('id', studentId).maybeSingle(),
        db.from('routines').select('id, name, objetivo, days_per_week, duration_weeks')
            .eq('gym_id', gymId).is('deleted_at', null).order('created_at', { ascending: false })
    ]);
    ...
    if (activeProg) {
        ...
        const rmsHTML = Object.entries(activeProg.rm_values || {})
            .map(([k, v]) => `<span style="font-size:10px;font-weight:700;color:#64748B;background:#0B1218;padding:2px 8px;border-radius:6px;border:1px solid #1E293B;font-family:'IBM Plex Mono',monospace">${k.toUpperCase()}: ${v}kg</span>`)
            .join(' ');
        html += `
    ...
    if (!assignModal) {
        assignModal = new window.ProgramAssignModal({
            gymId,
            db,
            onSuccess: async () => { await renderTabRutina(studentId, studentName); }
        });
    }

    const openModal = () => assignModal.open({ preStudent: { id: studentId, full_name: studentName } });
    ...
    btnSave?.addEventListener('click', async () => {
        ...
        if (error) {
            if (feedback) { feedback.textContent = 'No se pudo guardar.'; feedback.style.color = '#F87171'; feedback.style.display = 'block'; }
        } else {
            if (feedback) { feedback.textContent = selectedRoutineId ? 'Rutina asignada correctamente.' : 'Rutina quitada.'; feedback.style.color = '#34D399'; feedback.style.display = 'block'; }
...
function setupProfilePanel() {
    ...
    document.getElementById('panel-btn-editar')?.addEventListener('click', () => {
        if (activePanelStudentId) openEditAlumno(activePanelStudentId);
    });
    ...
    document.getElementById('cert-file-input')?.addEventListener('change', async (e) => {
        ...
        try {
            ...
            if (url) {
                await db.from('students').update({ medical_certificate_url: url }).eq('id', activePanelStudentId);
                const s = allStudents.find(x => x.id === activePanelStudentId);
            ...
            if (errorEl) { errorEl.textContent = 'Error al subir: ' + err.message; errorEl.classList.remove('hidden'); }
        } finally {
...
window.openProfilePanel = function (studentId) {
    ...
    document.getElementById('panel-objetivo-label').textContent = objInfo ? objInfo.label : 'Sin objetivo definido';
    ...
    // Tab MEMBRESÍAS
    const memberships = s.memberships?.sort((a, b) => new Date(b.end_date) - new Date(a.end_date)) || [];
    ...
    if (memEl) {
        if (!memberships.length) {
            ...
            memEl.innerHTML = memberships.map(m => {
...
function setupModalAlumno() {
    ...
    window.openNewAlumno = function () {
        document.getElementById('modal-alumno-title').innerHTML = 'Nuevo <span class="text-primary">Alumno</span>';
    ...
    window.openEditAlumno = function (id) {
        const s = allStudents.find(x => x.id === id);
        if (!s) return;
        document.getElementById('modal-alumno-title').innerHTML = 'Editar <span class="text-warning">Alumno</span>';
    ...
    function closeModal() { window.tfUtils.hideModal('modal-alumno'); }
    ...
    async function handleSave(e) {
        ...
        if (id) {
            ...
            ({ error } = await db.from('students').insert({ ...payload, gym_id: gymId, membership_status: 'pendiente' }));
        }
        ...
        window.tfUtils.toast(id ? 'Alumno actualizado' : 'Alumno creado');
        closeModal();
        await loadStudents();
    }
...
function setupModalMembresia() {
    ...
    function renderPlanButtons() {
        ...
        if (!gymMembershipPlans.length) {
            planButtonsContainer.innerHTML = '<p class="text-xs text-danger col-span-3">No hay planes disponibles.</p>';
            return;
        }
        planButtonsContainer.innerHTML = gymMembershipPlans.map((plan) => `
    ...
    function selectPlan(planKey, amount = null) {
        document.querySelectorAll('#membresia-plan-buttons .plan-btn').forEach((b) =>
    ...
    window.openMembresiaForStudent = async function (studentId, studentName) {
        try {
            ...
            if (errorEl) { errorEl.textContent = 'No se pudieron cargar los planes del gimnasio.'; errorEl.classList.remove('hidden'); }
            return;
        }
        renderPlanButtons();
        ...
        document.querySelectorAll('#membresia-plan-buttons .plan-btn').forEach(b => b.classList.remove('border-primary', 'text-primary', 'bg-primary/10'));
        errorEl?.classList.add('hidden');
        window.tfUtils.showModal('modal-nueva-membresia');
    };

    function closeModal() { window.tfUtils.hideModal('modal-nueva-membresia'); }
    ...
    async function handleSave(e) {
        ...
        if (!plan) {
            if (errorEl) { errorEl.textContent = 'Seleccioná un plan.'; errorEl.classList.remove('hidden'); }
            return;
        }
...
function setupModalEliminar() {
    ...
    function closeModal() { window.tfUtils.hideModal('modal-eliminar'); studentToDelete = null; }
    ...
    async function confirmarEliminar() {
        ...
        if (!error) {
            window.tfUtils.toast('Alumno eliminado');
            closeModal();
