...
function prepareDist(outputDir = 'dist') {
  ...
  rootFiles
    ...
    .forEach((htmlFile) => fs.copyFileSync(htmlFile, path.join(outputDir, htmlFile)));
  ...
  if (fs.existsSync('js')) {
    fs.cpSync('js', path.join(outputDir, 'js'), { recursive: true });
  }
}
...
function main() {
  try {
    ...
    prepareDist(process.env.BUILD_OUTPUT_DIR || 'dist');
    writeConfigFile(outputPath, content);
    if (!path.isAbsolute(outputPath)) {
      writeConfigFile(path.join(process.env.BUILD_OUTPUT_DIR || 'dist', outputPath), content);
    }
  } catch (error) {
...
module.exports = { validateEnv, buildSupabaseConfig, writeConfigFile, prepareDist, main };
